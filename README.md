...
- `public/Nicholas-Kimball-Resume.pdf` is generated from the source `.docx` by
  ...
  script.
- `public/og.png` and `public/og-playlists.png` are the social-preview images (LinkedIn, Slack,
  iMessage). LinkedIn won't render a link card without an `og:image`. They're generated by
  `scripts/build_og_images.py` — rerun it if the headline numbers change.
- `public/playlists-evals.html` has social-preview `<meta>` tags injected after its `<title>` that
  the source report doesn't carry, and is wrapped in a full `<!doctype html>` / `<head>` / `<body>`
  skeleton the source lacks (it was written for a viewer that adds one). Link scrapers only read
  meta tags from a real `<head>`. If you re-copy the report from the playlists repo, redo both.
